...
const SaleAndRecoveryForm = ({
  setLoader,
  setSaleAndRecoveryDetails,
  sideDrawerOpened,
}) => {
  const {
    register,
  ...
  useEffect(() => {
    const getSaleAndRecoveryData = async () => {
      try {
        setLoader(true);
  ...
  //Form submit handler
  const handleSaleAndRecoveryFormSubmit = async (data) => {
    try {
      setLoader(true);
      const response = await getSalesAndRecoveryDetails(data);
      setLoader(false);
      ...
      if (response.data.error) {
        ...
        setSaleAndRecoveryDetails(response.data.data);
      }
    } catch (error) {
      setLoader(false);
      if (error.request.status === 401) {
  ...
  return (
    <form
      onSubmit={handleSubmit(handleSaleAndRecoveryFormSubmit)}
        ...
        <div>
          ...
          <div className="mt-1 flex justify-center">
            <input
              ...
              className="mr-2"
            />
            <label htmlFor="Customer" className="inline-block mr-4">
              Customers
            </label>
            <input
              type="radio"
              ...
              className="mr-2"
            />
            <label htmlFor="sls" className="inline-block">
              Sales Persons
            </label>
          </div>
          {errors.type && (
...
export default SaleAndRecoveryForm;
